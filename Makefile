BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Die_Forschungsreise_des_Herzogs_der_Abruzzen_nach_dem_Eliasberge"
TITLE_NAME = "Die Forschungsreise des Herzogs der Abruzzen nach dem Eliasberge."
...
$(BUILD)/epub/$(OUTPUT_FILENAME).epub: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(CSS_FILE_KINDLE) $(IMAGES) \
	...
	mkdir -p $(BUILD)/epub
	$(PANDOC) $(ARGS) --from markdown+raw_html+fenced_divs+fenced_code_attributes+bracketed_spans+multiline_tables+grid_tables+footnotes --to epub+raw_html --resource-path=$(IMAGES_FOLDER) --epub-cover-image=$(COVER_IMAGE) -o $@  $(PREFACE_EPUB) $(CHAPTERS)
